...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
    ...
    // Query
	$pocket_q = $dbs->query('SELECT i.item_code,i.inventory_code,i.biblio_id,i.call_number,b.title FROM item AS i LEFT JOIN biblio AS b ON b.biblio_id=i.biblio_id WHERE i.item_code IN('.$item_ids.')');
    $pocket_datas = array();
    ...
    $sysconf['print']['pocket']['libraryname'] = "UPT Perpustakaan Jacob Nahuway Library";
    $sysconf['print']['pocket']['schoolname'] = "STT Bethel Ambon";
    ...
    foreach ($chunked_pocket_arrays as $pocket_rows) {
        foreach ($pocket_rows as $pocket) {
          ...
          $html_str .= '<span style="margin-left:60px;">: '.$pocket['inventory_code'].'</span>';
          $html_str .= '<br>';
...
<div class="menuBoxInner printIcon">
	...
	<div class="sub_section">
		<div class="btn-group">
		<a target="blindSubmit" href="<?= $_SERVER['PHP_SELF'] . '?' . httpQuery(['action' => 'clear']) ?>" class="notAJAX btn btn-default" style="color: #f00;"><i class="glyphicon glyphicon-trash"></i>&nbsp;<?php echo __('Clear Print Queue'); ?></a>
		<a target="blindSubmit" href="<?= $_SERVER['PHP_SELF'] . '?' . httpQuery(['action' => 'print']) ?>" class="notAJAX btn btn-default"><i class="glyphicon glyphicon-print"></i>&nbsp;<?php echo __('Print Pocket for Selected Data'); ?></a>
